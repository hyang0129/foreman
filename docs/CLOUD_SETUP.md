...
2. Copy `cloud.json` from a paired machine's `~/.foreman` into the new machine's `~/.foreman` (or its `FOREMAN_HOME`), and make it private: `chmod 600 ~/.foreman/cloud.json`. The daemon accepts only a regular file (not a symlink) owned by the user running it, with mode 0600. Any other file counts as present-but-invalid, and that machine runs no Coordinator until it is fixed.
...
4. Optionally set `FOREMAN_MACHINE_NAME` in the daemon's environment to name the machine on the Coordinator's info screen and in the **Move Coordinator…** dialog (default: the short hostname). On a Mac, set it when you run `npm run service:install`: the installer copies it (and `FOREMAN_PM_HUNG_MS`) into the installed service, and refuses to install if either is invalid. `status`, `uninstall` and `restart` do not check them. The daemon saves the name to `machine.json`, so to rename later run `npm run service:uninstall`, then `service:install` again with the new name. Installing without it keeps the name already saved.
...
Instead of `cloud.json`, a daemon can take the pairing from the environment: set **both** `FOREMAN_RELAY_URL` (the `url` from `cloud.json`, an HTTPS origin) and `FOREMAN_HOST_TOKEN` (its `token`). The environment takes precedence over `cloud.json`. Setting only one of them is invalid, and that machine runs no Coordinator. The macOS service installer does not capture these variables, so an installed service needs `cloud.json`.

The new machine connects as a **standby**: the Coordinator stays on the machine that already runs it, and the hosted app keeps talking to that machine only. The new machine appears in the **Move Coordinator…** dialog on the Coordinator's info screen. See [More than one machine](EXECUTION_HOST.md#more-than-one-machine) and [Moving the Coordinator](EXECUTION_HOST.md#moving-the-pm).

## Upgrading to portable Coordinator memory

This release moves the Coordinator's memory into the relay's Durable Object ([details](DESIGN.md#pm-memory-and-the-pm-host)). Deploy the Worker first, then restart the daemons:
...
2. Restart the daemon of the machine whose Coordinator memory you want to keep (`npm run service:restart`). The first upgraded daemon to connect becomes the Coordinator host, and it imports its own `memory/PROJECTS.md`, `memory/LOG.md` and `pm/settings.json` model into the relay once. The import writes `memory/.imported.json` and leaves the source files untouched. First importer wins: a machine that becomes the Coordinator host later imports nothing.
...
A daemon that has not been upgraded keeps working against the new Worker until the first upgraded daemon connects. From then on the relay sends everything to the Coordinator host. An upgraded daemon against the old Worker never receives a Coordinator assignment, so it runs no Coordinator (fail closed) until the Worker is deployed.

After the restart the Coordinator conversation starts empty. Earlier conversations are not carried over. `pm/history.jsonl`, `pm/session` and `pm/session.quarantine.jsonl` are no longer used and are safe to delete by hand.
...
- The pinned Coordinator keeps no conversation history. Its memory lives in the relay's Durable Object. A Coordinator message is recorded before it is sent; if its machine restarts or the Coordinator moves before it finishes, the message is reported once as uncertain and never replayed. See [Coordinator memory and host](DESIGN.md#pm-memory-and-the-pm-host).
- The hosted app talks to one machine at a time: the active Coordinator host. That machine must remain awake, connected, and logged in. A hosted page cannot execute local processes while it is off; move the Coordinator to another online machine instead ([Moving the Coordinator](EXECUTION_HOST.md#moving-the-pm)).
